 ...
 * area должно удовлетворять одному из 3 свойств:
 * 1) прямоугольно-выпуклое - hasProperty("RectConvex"), или hasProperty("QuasiRectConvexV"), или hasProperty("QuasiRectConvexH")
 * 2) связное объединение прямоугольно-выпуклых - Union или BigUnion и каждая компонента - прямоугольно-выпуклая
 * 3) объединение непересекающихся компонент типа 2 - Union или BigUnion со включенным isDisjoint
 * */
...
/** значения переменных x, y, от которых будут отсчитываться остальные */
  class BaseNumbers
  {
	  ...
	  void addDiffByXSquares(Difference2Info diff_info, double diffs[][], RectangleQuadraticTask task) {
	  ...
	  void addDiffByYSquares(Difference2Info diff_info, double diffs[][], RectangleQuadraticTask task) {
 		  // TODO: что делать, если s не делится на число пикселей?
		  int nx = getSizeX() + 1,
			  ny = getSizeY() + 1; // количество значений	  
 ...
 // восстановление функции по сдвигу по с заданным шагом и величиной сдвига
  ...
  public void restoreByShift(Difference2Info[] diffs) {
	  ...
	  int mx = (int)(s/getStepX() + 1e-10),
		  my = (int)(s/getStepY() + 1e-10); // количество переменных
	  //	  System.out.printf("s=%f, delta=%f, n=%d m=%d", s, delta, n, m);
	  // создаём массив разностей:
	  BaseNumbers numbers = new BaseNumbers(mx, my);
