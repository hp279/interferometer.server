 ...
 * свойств: 1) прямоугольно-выпуклое - hasProperty("RectConvex") 2) связное
 * объединение прямоугольно-выпуклых - Union или BigUnion и каждая компонента -
 * прямоугольно-выпуклая 3) объединение непересекающихся компонент типа 2 -
 * Union или BigUnion со включенным isDisjoint
 * */
            ...
            for (int j1y = 0; j1y < my; ++j1y)
                for (int j2x = Math.max(j1x - dx, 0); j2x < Math.min(j1x + dx,
                    ...
                    for (int j2y = Math.max(j1y - dy, 0); j2y < Math.min(j1y
                        ...
                        if (j1 == j2) // сравнивать переменную саму с собой нет
                                      // смысла
                        {
