...
public class InterferometerRestoreFunction extends TableFunction2
{
  ...
  public InterferometerRestoreFunction(Type type, double minx, double maxx, double miny, double maxy, int m, int n, 
		  										  double a, double b, double lambda, double deltaz)
  {
  ...
  public InterferometerRestoreFunction(Type type, double a, double b, double lambda)
  {
	  this(type, 0, 0, 0, 0, 0, 0, a, b, lambda, 1);
  }
  public InterferometerRestoreFunction(Type type, String filename) throws Exception
  {
	  this.type = type;
	  this.load(filename);
	  this.evalParams();
...
/*	  this.iterate(new TableAction<TableFunction2>(sumborders)
  ...
  /** Определяем максимум и минимум и, исходя из этого, приводим к отрезку [-1;1] */
  private void normalize()
  {
	  final double min = this.aggregate(DoubleAggregator.MIN), 
			  	   max = this.aggregate(DoubleAggregator.MAX);
	  System.out.printf("\nmax=%f, min=%f", max, min);
	  ...
	  try
	  {
		...
		//	 max = color_model.getColorSpace().getMaxValue(0); 
		  ...
		  for(int j=0; j<= getSizeY(); ++j)
		  {
			  ...
			  setValue(i, j, value);
			  // TODO: уточнить, из какого бэнда брать значения - или брать альфу???
		  }
		  setDefined();
		  normalize();
	  }
