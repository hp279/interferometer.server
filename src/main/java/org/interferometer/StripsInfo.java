...
public class StripsInfo {
    ...
    LinkedList<Border> min_borders, max_borders, nil_borders;
    ...
    public StripsInfo(InterferometerRestoreFunction function,
            StripsOptions options) {
    ...
    private Border findBorder(Border.Type type, IntPoint point) {
        switch (type) {
        case Max:
            return ArraysEx.find(this.max_borders, new Border(point));
        case Min:
            return ArraysEx.find(this.min_borders, new Border(point));
        case Nil:
            return ArraysEx.find(this.nil_borders, new Border(point));
        default:
            return null;
        }
    }
    ...
    private Border addBorder(Border.Type type, IntPoint pt) {
        ...
        switch (type) {
        case Max:
            this.max_borders.add(result);
        case Min:
            this.min_borders.add(result);
        case Nil:
            this.nil_borders.add(result);
        }
        return result;
    }
    ...
    private void removeBorder(Border b) {
        switch (b.getType()) {
        case Max:
            this.max_borders.remove(b);
        case Min:
            this.min_borders.remove(b);
        case Nil:
            this.nil_borders.remove(b);
        }
    }
    ...
    private int getNearNeighbors(IntPoint last_point, IntPoint point,
        ...
        if (last_point != null)
            System.out.printf(
                    ...
                    last_point.getX(), last_point.getY(), point.getX(),
                    point.getY());
    ...
    private void createBorderLine(Border.Type type, Border newborder,
        ...
        // TODO: сделать удаление из массива предыдущей точки линии, если мы не
        // предполагаем кольцевания
        border_cycle: while (true) {
            ...
            case 1:
                ...
                System.out.printf("\n neighbor = (%d, %d)", current_point
                        .getNeighbor(neighbors[0]).getX(), current_point
                        .getNeighbor(neighbors[0]).getY());
                temp_border = this.findBorder(type,
                        current_point.getNeighbor(neighbors[0]));
                linkBorder(newborder, temp_border);
                break border_cycle;
    ...
    private void createMinBorders() {
        evaluateBorders(options.getMinOptions(), -1, 0, 0,
                Utils.sqr(function.getXCoef()), PixelType.MayBeMin);
        createBorderLines(Border.Type.Min);
    }

    // определяем максимумы функций там, где это возможно:
    private void createMaxBorders() {
        evaluateBorders(options.getMaxOptions(), 1, 0, 0,
                -Utils.sqr(function.getXCoef()), PixelType.MayBeMax);
        createBorderLines(Border.Type.Max);
    }

    private void createNilBorders() {
        if (options.mustCreateNilBorders()) {
            evaluateBorders(options.getNilOptions(), 0, function.getXCoef(), 0,
                    0, PixelType.MayBeNil);
