...
public class BordersInfo {
    ...
    ListOrderedSet<Border> min_borders, max_borders, nil_borders;
    IntPoint far_point; // "бесконечно удалённая точка" - с ней соединяются все
                        // границы, уходящие за пределы поля
    StripsOptions options;

    public enum Status {
        NotRestored, BordersRestored, BordersLinked, Error;
    }
    ...
    public enum PixelType {
        ...
        PossibleNilAndBegin, // точка равенства 0, являющаяся началом или концом
                             // какой-то линии
        IThinkNil; // окончательно идентифицирована как точка, где функция равна
                   // 0, и часть линии уровня
    ...
    public BordersInfo(InterferometerRestoreFunction function,
            StripsOptions options) {
    ...
    private void evaluateBorders(StripsOptions.EvaluateOptions opts,
            double must_value, double must_grad, double must_det,
            double must_trace, PixelType eval_type) {
        for (int i = 0; i < m; ++i)
            for (int j = 0; j < n; ++j) {
                ...
                if (function.hasArgument(x, y)) {
                    ...
                    if (opts.evalFunction(val, must_value)) {
                        if (opts.evalDiff(function, x, y, must_grad)
                                && opts.isConstantCenter(function, x, y)) {
                            if (opts.evalDiff2(function, x, y, must_det,
                                    must_trace))
    ...
    private Border findBorder(Border.Type type, IntPoint point) {
        return ArraysEx.find(this.getBordersByType(type), new Border(this,
                point));
    }
    ...
    private int getNearNeighbors(IntPoint last_point, IntPoint point,
            PixelType type, IntPoint.Neighbor neighbors[]) {
        // if(last_point != null)
        // System.out.printf("\n getNearNeighbors: last_point=(%d, %d), point=(%d, %d)",
        // last_point.getX(), last_point.getY(), point.getX(), point.getY());
        int result = point.getNearNeighborsCount(this.evaluations, type,
                neighbors);
        int from = 0, real_from = 0;
        for (;; ++from, ++real_from) {
            // System.out.println(real_from);
            // System.out.println(array[real_from]);
            while (real_from < result
                    && point.getNeighbor(neighbors[real_from]).equals(
                            last_point))
                real_from++;
            if (real_from >= result)
                break;
    ...
    private int getFarNeighbors(IntPoint last_point, IntPoint point,
            PixelType type, IntPoint.Neighbor neighbors[]) {
        int result = point.getFarNeighborsCount(this.evaluations, type,
                neighbors);
        int from = 0, real_from = 0;
        for (;; ++from, ++real_from) {
            // System.out.println(real_from);
    ...
    private void createBorderLine(Border.Type type, Border newborder,
            IntPoint current_point) {
        System.out.printf("\n Begin: (i, j) = (%d, %d)", current_point.getX(),
                current_point.getY());
        Border temp_border;
        IntPoint temp_pt;
        ...
        border_cycle: while (true) {
            ...
            int good_neighbors = getNearNeighbors(newborder.getPrevLast(),
                    current_point, type.getPossibleBeginType(), neighbors);
            System.out.printf("\n (i, j) = (%d, %d); good_neighbors = %d",
                    current_point.getX(), current_point.getY(), good_neighbors);
            int bad_neighbors = 0;
            switch (good_neighbors) {
            case 0:
                good_neighbors = getNearNeighbors(newborder.getPrevLast(),
                        current_point, type.getMayBeType(), neighbors);
                switch (good_neighbors) {
                case 0:
                    // проверяем дальних соседей:
                    // TODO: сделать учёт собственного вектора матрицы 2-х
                    // производных
                    bad_neighbors = getFarNeighbors(newborder.getPrevLast(),
                            current_point, type.getPossibleType(), neighbors);
                    // в середину линии мы утыкаться не должны - в этом случае
                    // делаем откат:
                    if (bad_neighbors > 0) {
                        this.decreaseBorder(newborder);
                        break border_cycle;
                    }
                    good_neighbors = getFarNeighbors(newborder.getPrevLast(),
                            current_point, type.getPossibleBeginType(),
                            neighbors);
                    switch (good_neighbors) {
                    case 0:
                        good_neighbors = getFarNeighbors(
                                newborder.getPrevLast(), current_point,
                                type.getMayBeType(), neighbors);
                        switch (good_neighbors) {
                        case 1:
                        ...
                        case 2:
                            if (newborder.isOnlyBegin()) {
                                IntPoint pt1 = current_point
                                        .getNeighbor(neighbors[0]), pt2 = current_point
                                        .getNeighbor(neighbors[1]);
                    ...
                    case 1:
                        temp_border = this.findBorder(type,
                                current_point.getNeighbor(neighbors[0]));
                        linkBorder(newborder, temp_border);
                        break border_cycle;
                    case 2:
                        if (newborder.isOnlyBegin()) {
                            Border border1 = this.findBorder(type,
                                    current_point.getNeighbor(neighbors[0])), border2 = this
                                    .findBorder(type, current_point
                                            .getNeighbor(neighbors[1]));
                            border1.reverse();
                ...
                case 2:
                    if (newborder.isOnlyBegin()) {
                        IntPoint pt1 = current_point.getNeighbor(neighbors[0]), pt2 = current_point
                                .getNeighbor(neighbors[1]);
                        newborder.addBeginPixel(pt1);
                        increaseBorder(newborder, pt2);
                        current_point = pt2;
            ...
            case 1:
                ...
                // System.out.printf("\n neighbor = (%d, %d)",
                // current_point.getNeighbor(neighbors[0]).getX(),
                // current_point.getNeighbor(neighbors[0]).getY());
                temp_border = this.findBorder(type,
                        current_point.getNeighbor(neighbors[0]));
                linkBorder(newborder, temp_border);
                break border_cycle;
            case 2:
                if (newborder.isOnlyBegin()) {
                    Border border1 = this.findBorder(type,
                            current_point.getNeighbor(neighbors[0])), border2 = this
                            .findBorder(type,
                                    current_point.getNeighbor(neighbors[1]));
                    border1.reverse();
                    linkBorder(border1, newborder);
                    linkBorder(border1, border2);
                    break border_cycle;
    ...
    public double getPointDifficult(IntPoint pt, Border.Type type) {
        if (this.function.hasArgumentInt(pt) && !this.getType(pt).inLine()) {
            return this.options.getOptions(type).howBadValue(function,
                    function.getArgument1(pt.getX()),
                    function.getArgument2(pt.getY()), type.getValue());
        } else
            return Double.POSITIVE_INFINITY;
    }

    /** соединяем 2 точки */
    public Pair<Border, Double> getShortestBorder(IntPoint a, IntPoint b,
            Border.Type type) {
        ...
        while (!current_point.equals(b)) {
            ...
            Pair<IntPoint.Neighbor, IntPoint.Neighbor> neighbors = IntPoint.Neighbor
                    .getNearestDirections(direction);
            IntPoint pt1 = current_point.getNeighbor(neighbors.first), pt2 = current_point
                    .getNeighbor(neighbors.second);
            double dist1 = getPointDifficult(pt1, type), dist2 = getPointDifficult(
                    pt2, type);
            if (result.canBeAdd(pt1)
                    && (dist1 < dist2 || !result.canBeAdd(pt2))) {
                result.addPixel(pt1);
                current_point = pt1;
                dist += dist1;
    ...
    /**
     * пытаемся соединить точку с каким-нибудь бордером или с краем области
     * определения
     */
    /*
     * private void linkPoint(IntPoint pt, Border.Type type, Set<Border>
     * borders) { // действуем тупо: сперва пробуем соединить с границей, потом
     * - с другими бордерами Border border_to_finish =
     * this.getShortestBorder(pt, type).first; if(border_to_finish != null) {
     * this.setType(pt, type.getIThinkType()); border_to_finish.setType(type);
     * borders.add(border_to_finish); } else { Iterator<Border> itr =
     * borders.iterator();; while(itr.hasNext()) { Border border2 = itr.next();
     * IntPoint pt21 = border2.getFirst(), pt22 = border2.getLast(); Border
     * border_between = this.getShortestBorder(pt, pt21, type).first;
     * if(border_between != null) { this.setType(pt, type.getIThinkType());
     * this.setType(pt21, type.getIThinkType()); border_between.setType(type);
     * borders.add(border_between); break; } else { border_between =
     * this.getShortestBorder(pt, pt22, type).first; if(border_between != null)
     * { this.setType(pt, type.getIThinkType()); this.setType(pt22,
     * type.getIThinkType()); border_between.setType(type);
     * borders.add(border_between); break; } } } } }
     */

    /** соединяем границы, чтобы они удовлетворяли условию замкнутости */
    private void linkBorders(Border.Type type) {
        BorderGraph graph = new BorderGraph(this, type,
                this.getBordersByType(type));
        ...
        if (newborders != null) {
            ...
            for (Border b : newborders)
                b.setFinal();
            // TODO: сделать расщепление поля, чтобы можно было пробовать
            // соединять бордеры по-разному на отдельных полях
        }
        /*
         * Iterator<Border> itr = borders.iterator(); while(itr.hasNext()) {
         * Border border1 = itr.next(); IntPoint pt11 = border1.getFirst(), pt12
         * = border1.getLast(); if(!pt11.equals(far_point) && this.getType(pt11)
         * != type.getIThinkType()) linkPoint(pt11, type, newborders);
         * if(!pt12.equals(far_point) && this.getType(pt12) !=
         * type.getIThinkType()) linkPoint(pt12, type, newborders); }
    ...
    public void createBorders() {
        ...
        if (status == Status.BordersRestored) {
            // TODO: сделать линковку в цикле, пока они не станут линковаться
            // хорошо
            linkBorders(Border.Type.Nil);
            linkBorders(Border.Type.Max);
            linkBorders(Border.Type.Min);
            this.status = Status.BordersLinked; // TODO: проверить, что всё
                                                // хорошо, иначе Error
        }
    }
}
