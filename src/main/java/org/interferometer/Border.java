...
public class Border implements Comparable<Border> {
    ...
    public static class EdgePoint extends Pair<IntPoint, Boolean> implements
            Comparable<EdgePoint> {
        ...
        public int compareTo(EdgePoint o) { // true идёт перед false
            return first.compareTo(o.first) * 4
                    - Boolean.compare(second, o.second);
        }
    }
    ...
    public boolean canBeAdd(IntPoint pt) {
        IntPoint last_pt = this.getLast(), left_pt = pt
                .getNeighbor(IntPoint.Neighbor.Left), up_pt = pt
                .getNeighbor(IntPoint.Neighbor.Up), right_pt = pt
                .getNeighbor(IntPoint.Neighbor.Right), down_pt = pt
                .getNeighbor(IntPoint.Neighbor.Down);
        return this.field.hasArgument(pt)
                && !this.field.getType(pt).inLine()
                && (!this.field.hasArgument(left_pt) || left_pt.equals(last_pt) || !this.field
                        .getType(left_pt).inLine())
                && (!this.field.hasArgument(up_pt) || up_pt.equals(last_pt) || !this.field
                        .getType(up_pt).inLine())
                && (!this.field.hasArgument(right_pt)
                        || right_pt.equals(last_pt) || !this.field.getType(
                        right_pt).inLine())
                && (!this.field.hasArgument(down_pt) || down_pt.equals(last_pt) || !this.field
                        .getType(down_pt).inLine());
    }
    ...
    public void addPixel(IntPoint pt) {
        ...
        if (this.field != null && this.type != Type.Empty)
            this.field.setType(pt,
                    is_final ? type.getIThinkType() : type.getPossibleType());
    }
    ...
    public void addBeginPixel(IntPoint pt) {
        if (this.field != null && this.type != Type.Empty) {
            this.field.setType(this.pixels.getFirst(),
                    is_final ? type.getIThinkType() : type.getPossibleType());
            this.field.setType(
                    pt,
                    is_final ? type.getIThinkType() : type
                            .getPossibleBeginType());
        }
    ...
    public void reverse() {
        ...
        if (this.field != null && this.type != Type.Empty) {
            field.setType(pixels.getLast(), is_final ? type.getIThinkType()
                    : type.getPossibleType());
            field.setType(pixels.getFirst(), is_final ? type.getIThinkType()
                    : type.getPossibleBeginType());
        }
    }

    /**
     * Устанавливает правильное направление, исходя из начальной или конечной
     * точки
     */
    public Border setOrientation(IntPoint firstpt, IntPoint lastpt) {
        if ((firstpt != null && this.getFirst() != firstpt)
                || (lastpt != null && this.getLast() != lastpt))
            reverse();
        return this;
    }
    ...
    public void addEnd(Border b, boolean link_point) {
        ...
        // type.getPossibleType();
        if (this.field != null && this.type != Type.Empty)
            this.field.setType(b.pixels.getFirst(),
                    is_final ? type.getIThinkType() : type.getPossibleType());
        if (link_point) {
