...
public class IntPoint implements Comparable<IntPoint> {
    ...
    public enum Neighbor {
        ...
        Right(0), RightUp(1), Up(2), LeftUp(3), Left(4), LeftDown(5), Down(6), RightDown(
                7);
        ...
        static public Pair<Neighbor, Neighbor> getNearestDirections(
                Vector2 vector) {
    ...
    /**
     * Считает количество ближних соседей, которые равны value, и записывает в
     * values
     */
    private <T> int getNearNeighborsCount(T array[][], T value,
            Neighbor values[], int from) {
     ...
     * values
     */
    public <T> int getNearNeighborsCount(T array[][], T value,
            Neighbor values[]) {
        return getNearNeighborsCount(array, value, values, 0);
    }

    /**
     * Считает количество дальних соседей, которые равны value, и записывает в
     * values
     */
    private <T> int getFarNeighborsCount(T array[][], T value,
            Neighbor values[], int from) {
        if (x + 1 < array.length && y + 1 < array[x + 1].length
                && array[x + 1][y + 1].equals(value))
            values[from++] = Neighbor.RightUp;
        if (x > 0 && y + 1 < array[x - 1].length
                && array[x - 1][y + 1].equals(value))
    ...
    /**
     * Считает количество соседей, которые равны value, и записывает в values
     * последнего
     */
