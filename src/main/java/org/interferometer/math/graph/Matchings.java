...
public class Matchings {
    ...
    public static <V, E, N extends Number> void getMaxMatching(
            Graph<V, E> graph, Graph<V, E> result) {
    ...
    public static <V, E, N extends Number> void getMaxMatching(
            Graph<V, E> graph, Transformer<E, N> costs, Graph<V, E> result) {
        System.out.printf("\bGraph has %d edges.", graph.getEdgeCount());
        // пока что действуем тупо: соединяем каждую точку с той, которая
        // максимальна по стоимости ребра
        for (V v1 : graph.getVertices()) {
