...
public class Test {
    // проверка, как работает квадратичное программирование
    static void testSolver() {
        ...
        // добавляем линейные ограничения
        for (int j = 0; j < m; ++j) {
            Expression ex = model.addEmptyLinearExpression(String.format("%d",
                    j + 1));
            for (int i = 0; i < n; ++i) {
                double aij = (i == j ? 1 : 0); // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        objex.setContributionWeight(new BigDecimal(1)); // 1 - минимизация, -1 -
                                                        // максимизация!
        ...
        // Если только равенства - работает LagrangeSolver. Он сводит решение к
        // системе линейных уравнений
        // и вызывает EquationSystemSolver (решение в 1 шаг).
        // Если есть неравенства - ActiveSetSolver. Он на каждом шаге
        // устанавливает активные неравенства
        // и использует LagrangeSolver, в котором активные неравенства
        // заменяются на равенства, а остальные откидываются.
        ...
        case FEASIBLE:
            System.out.printf("Feasible!\n");
            // Выводим его на экран:
            for (int i = 0; i < vector.getRowDim(); ++i)
                System.out.printf("x%d = %f\n", i + 1, vector
                        .toBigDecimal(i, 0).doubleValue());
            break;
        }
    }

    // проверка восстановления 1-мерной функции по разности
    static void testRestore1() {
        ...
        try {
            ...
            AbstractFunction1 helder = new AbstractFunction1() // предполагаем,
                                                               // что функция
                                                               // удовлетворяет
                                                               // условию
                                                               // Гёльдера с
            ...
            RestoredTableFunction fun2 = new RestoredTableFunction(0, 100, 100,
                    helder);
            fun2.restoreByShift(diff);
            System.out.printf("Function is restored!");
            // записываем восстановленную функцию в output.txt - должно
            // получиться близко к исходной функции
    ...
    static FlatSet getTestSet(double xcenter, double ycenter, double r, double R) {
        ...
        FlatSet biground = new Round(xcenter, ycenter, R), smallround = new Round(
                xcenter, ycenter, r);
        FlatSet circle = new Difference(biground, smallround);
        // сперва вернём верхнюю половину:
        Rectangle urect = new Rectangle(-Double.MAX_VALUE, -Double.MAX_VALUE,
                xcenter, Double.MAX_VALUE), drect = new Rectangle(xcenter,
    ...
    static void testRestore2(int size, int[] sarray, double h, double sigma) {
        ...
        try {
            ...
            for (int i = 0; i < difnum; ++i) {
                ...
                TableFunction2 tabledelta1f = new TableFunction2(s, size, 0,
                        size, size - s, size), tabledelta2f = new TableFunction2(
                        0, size, s, size, size, size - s);
                ...
                String filename1 = String.format("outputs%dd1.txt", i + 1), filename2 = String
                        .format("outputs%dd2.txt", i + 1);
            ...
            RestoredTableFunction2 fun2 = new RestoredTableFunction2(0, size,
                    0, size, size, size, helder, h);
            fun2.setArea(area);
            fun2.restoreByShift(diff);
            System.out.printf("Function is restored!");
            // записываем восстановленную функцию в output.txt - должно
    ...
    static void testInterfer(int size, int[] sarray, double h, double a,
            double b, double lambda, double deltaz, double sigma) {
        // создаём табличную функцию из значений в input.txt
        TableFunction2 fun = new TableFunction2(0, size, 0, size, size, size);
        // устанавливаем множество:
        ...
        try {
            ...
            for (int i = 0; i < difnum; ++i) {
                int s = sarray[i];
                AbstractFunction2 delta1f = new Interferometer1Function2(fun,
                        s, a, b, lambda);
                AbstractFunction2 delta2f = new Interferometer2Function2(fun,
                        s, a, b, lambda);

                // создаём восстанавливающие функции
                InterferometerRestoreFunction picture1 = new InterferometerRestoreFunction(
                        InterferometerRestoreFunction.Type.ByX, s, size, 0,
                        size, size - s, size, a, b, lambda, deltaz);
                picture1.setArea(delta1f.getArea());
                picture1.assign(delta1f);
                picture1.append(noise);
                picture1.createStrips();
                InterferometerRestoreFunction picture2 = new InterferometerRestoreFunction(
                        InterferometerRestoreFunction.Type.ByY, 0, size, s,
                        size, size, size - s, a, b, lambda, deltaz);
                ...
                // на всякий случай их записываем:
                String filename1 = String.format("outputs%di1.txt", i + 1), filename2 = String
                        .format("outputs%di2.txt", i + 1);
                FileOutputStream fdelta1out = new FileOutputStream(filename1);
            ...
            // восстанавливаем то, что в наименьшем прямоугольнике - то есть
            // размером s[difnum-1]
            int maxs = sarray[difnum - 1];
            RestoredTableFunction2 fun2 = new RestoredTableFunction2(maxs, size
                    - maxs, maxs, size - maxs, size - maxs, size - maxs,
                    helder, h);
            // fun2.setArea(area);
            fun2.restoreByShift(diff);
            System.out.printf("Function is restored!");
